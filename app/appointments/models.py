...
class Appointment(Base):
    ...
    Уникальность:
        Комбинация doctor_id и start_time должна быть уникальной (не допускается двойное бронирование).
    """
    ...
    __table_args__ = (
        UniqueConstraint("doctor_id", "start_time", name="unique_doctor_slot"),
    )

    def __repr__(self) -> str:
        """Строковое представление записи на приём."""
        start_str = (
            self.start_time.strftime("%Y-%m-%d %H:%M") if self.start_time else None
        )
        return (
